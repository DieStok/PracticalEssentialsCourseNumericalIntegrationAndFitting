...
fitNMQZeroDiffer <- fit(data=list(dataAndFitsFigureThree$DataexptAFig3R, dataAndFitsFigureThree$DataexptBFig3R), 
                    free=free, differ=differ, fun=log, odes = model, state = initialState,
                    parms = initialParams, lower=lower, pch=".",legend=FALSE, tstep=0.1, 
                    main="red A1 & red B1, N, m, and q0 differing", add=TRUE, ymin = 0, ymax = 1)
#summary(fitKVMDiffer) --> gives an error of covariance. This is because of the lag phase parameters, which we allowed to be fitted in our call.
fitNMQZeroDiffer$ssr
...
#summary(dataAndFitsFigureThree$FitexptBFig3R) --> #can't fit separately well. 
...
#note: should of course add up the residuals of the fit of Red (A1) and Red (B1) separately for a fair comparison!
differenceSSR = abs(fitNMQZeroDiffer$ssr-(dataAndFitsFigureThree$FitexptAFig3R$ssr + dataAndFitsFigureThree$FitexptBFig3R$ssr))
...
#Note: the below diffSSR variables all contain the differences of the combined fit to the two separate fits of Red and Red data from figure 3 A1 and B1

###
### Differing only in K and N
###

#order of parameters from fit: first one is for red A1, second one for red B1
resultKDiffOnlyRedRed  = fitExpt3WithDiffer(differ = c("K", "N"), mode = "redred")
fitKDiffOnlyRedRed     = resultKDiffOnlyRedRed$fit
diffSSRKDiffOnlyRedRed = resultKDiffOnlyRedRed$diffSSR

#Here you assume that the two differ in carrying capacity only. That's a bit strange: they're the same bacterial strain. Not a very useful thing to test!

###
### Differing only in v and N --> doesn't work, fit goes way off the rails
###

#resultVDiffOnlyRedRed  = fitExpt3WithDiffer(differ = c("v", "N"), mode = "redred")
#fitVDiffOnlyRedRed     = resultVDiffOnlyRedRed$fit
#diffSSRVDiffOnlyRedRed = resultVDiffOnlyRedRed$diffSSR

###
### Differing only in m and N
###

resultMDiffOnlyRedRed  = fitExpt3WithDiffer(differ = c("m", "N"), mode = "redred")
fitMDiffOnlyRedRed     = resultMDiffOnlyRedRed$fit
diffSSRMDiffOnlyRedRed = resultMDiffOnlyRedRed$diffSSR

#This or the one below seem to be the most logical choice in this case. Only one lag parameter need differ!

###
### Differing only in q0 and N
###

resultQZeroDiffOnlyRedRed  = fitExpt3WithDiffer(differ = c("q0", "N"), mode = "redred")
fitQZeroDiffOnlyRedRed     = resultQZeroDiffOnlyRedRed$fit
diffSSRQZeroDiffOnlyRedRed = resultQZeroDiffOnlyRedRed$diffSSR

#This works just fine, as does the above!

###
### Differing in K and v and N --> doesn't work, numerical integration fails
###

#resultKVDiffRedRed  = fitExpt3WithDiffer(differ = c("K", "v", "N"), mode = "redred") #This doesn't work --> get errors!
#fitKVDiffRedRed     = resultKVDiffRedRed$fit
#diffSSRKVDiffRedRed = resultKVDiffRedRed$diffSSR


###
### Differing in v and m and N
###

resultVMDiffRedRed  = fitExpt3WithDiffer(differ = c("v", "m", "N"), mode = "redred")
fitVMDiffRedRed     = resultVMDiffRedRed$fit
diffSSRVMDiffRedRed = resultVMDiffRedRed$diffSSR

###
### Differing in K and m and N
###

resultKMDiffRedRed  = fitExpt3WithDiffer(differ = c("K", "m", "N"), mode = "redred")
fitKMDiffRedRed     = resultKMDiffRedRed$fit
diffSSRKMDiffRedRed = resultKMDiffRedRed$diffSSR

###
### Differing in q0, m and N: 
###

resultQZeroMDiffRedRed  = fitExpt3WithDiffer(differ = c("q0", "m", "N"), mode = "redred")
fitQZeroMDiffRedRed     = resultQZeroMDiffRedRed$fit
diffSSRQZeroMDiffRedRed = resultQZeroMDiffRedRed$diffSSR


```
#Answer: all in all you see that only different N and q0 or m are fine. Only 2 different estimates, rather than 6! In this way we make much better use of the data than the authors did.
...
#In sum, you'd think that either only letting K, v and m, or even only K and m (barely a difference then) differ between red and green in experiment a and fitting the data together would be best. You can do with equal q0, r, and N.
```
